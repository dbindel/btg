...
rangeλ = [-1.5 0.3] 
rangeθs = [parsed_args["range1"] parsed_args["range2"]]
...
if parsed_args["test"]
    ...
    for i in 1:n_test
        ...
        # @info "Count, id_fail" count_test, id_fail
            ...
            PyPlot.savefig("exp_creep_failedpdf_$(n_train)_$(myquadtype[1])_rθ_$(Int(rangeθs[1]))_$(Int(rangeθs[2]))_failedID_$i.pdf")
            mgrid = similar(weightsTensorGrid)
            R = CartesianIndices(weightsTensorGrid)
            for I in R
                mgrid[I] = tgridm[I](x_test_i, Fx_test_i)
...
if parsed_args["validate"]
    ...
    PyPlot.savefig("exp_creep_LOOCV_$(n_train)_$(myquadtype[1])_rθ_$(Int(rangeθs[1]))_$(Int(rangeθs[2]))_$(parsed_args["fast"]).pdf")
