...
function optimizeUCB(cdf, cdf_gradient, cdf_hessian, lx, ux)
    ...
    lc = 0.24; uc = 0.26 #constraint on cdf by default, since we want 1 stdev 
    x0 = init_constrained_pt(cdf_fixed, lx, ux; quantile = 0.25)
    @info "x0 found with init_constr_point", x0
    @info "cdf_fixed(x0)", cdf_fixed(x0)
    df = TwiceDifferentiable(fun, fun_grad!, fun_hess!, reshape(x0, length(x0)))
    @info "lx" lx
    @info "ux" ux
    @info "lc" lc
    @info "uc" uc
